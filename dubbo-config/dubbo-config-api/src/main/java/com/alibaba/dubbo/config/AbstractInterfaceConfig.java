...
public abstract class AbstractInterfaceConfig extends AbstractMethodConfig {
    ...
    //将application，regiestries，path，dubbo，等数据，拼接到map，并将regiestries的address正则匹配生成URL，匹配规则multicast://224.5.6.7:1234；0.0.0.0
    protected List<URL> loadRegistries(boolean provider) {
        checkRegistry();//检测registrys是否为空，如果为空，通过属性文件加载
        List<URL> registryList = new ArrayList<URL>();
        if (registries != null && registries.size() > 0) {
            for (RegistryConfig config : registries) {
                String address = config.getAddress();
                //如果address为空加载默认地址0.0.0.0
                if (address == null || address.length() == 0) {
                ...
                if (address != null && address.length() > 0
                    ...
                    appendParameters(map, application);//application的get，is方法，如果执行结果不为空，赋值给map，详情看该方法，
                    appendParameters(map, config);
                    map.put("path", RegistryService.class.getName());
                    map.put("dubbo", Version.getVersion());
                    map.put(Constants.TIMESTAMP_KEY, String.valueOf(System.currentTimeMillis()));//当前时间
                    ...
                    List<URL> urls = UrlUtils.parseURLs(address, map);//以map为参数，将address以";"匹配获取多个URL
                    for (URL url : urls) {
